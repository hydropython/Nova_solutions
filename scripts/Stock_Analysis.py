...
class StockAnalysis:
    ...
    def load_data(self, file_name):
        ...
        fig.update_layout(title=f'Raw Data ({file_name})', template='plotly_dark', height=800)
        fig.show()
    ...
    def display_portfolio_weights(self, portfolio_weights):
        ...
        fig.update_layout(title='Portfolio Weights', xaxis_title='Stock', yaxis_title='Weight', template='plotly_dark')
        fig.show()
    ...
    def visualize_data(self, df):
        ...
        fig.update_layout(title=f'Stock Price and Moving Averages ({df["File"].iloc[0]})', template='plotly_dark')
        fig.show()

        # Plot ATR
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df.index, y=df['ATR'], mode='lines', name='ATR', line=dict(color='purple')))
        fig.update_layout(title=f'ATR ({df["File"].iloc[0]})', template='plotly_dark')
        fig.show()
        ...
        fig.update_layout(title=f'RSI ({df["File"].iloc[0]})', template='plotly_dark')
        fig.show()
        ...
        fig.update_layout(title=f'MACD ({df["File"].iloc[0]})', template='plotly_dark')
        fig.show()
        ...
        fig.update_layout(title=f'Bollinger Bands ({df["File"].iloc[0]})', template='plotly_dark')
        fig.show()
